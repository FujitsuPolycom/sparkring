...
patched ring-safe NCCL fallback. [SIRCL transport](SIRCL.md) defines the
transport; [Architecture](ARCHITECTURE.md) defines the topology.
...
- `spark_transport/integrations/vllm/spark_tp4_stock_timing.py` times a fixed
  ...
  inventory.
- `spark_transport/experiments/q2r_phase_timing/live_installer.py` pins the
...
- the gate's own cost (`median(B2)`, maximised over ranks) is at most 10% of
  ...
  changed what it was placed there to isolate. `scripts/bench/collective_attribution.py`
...
- The multiplier depends on the algorithm. A ring all-reduce over four ranks
  ...
  decomposition [SIRCL transport](SIRCL.md) specifies for the four-Spark
...
- The rate. [Dual-port striping and graph-kernel probe record,
  2026-08-18](DUAL_PORT_STRIPING_PROBE_20260818.md) states, as an open item,
...
1. **Stream census (decisive for the negative case).** Record, for every
   ...
   timeline. `spark_tp4_stock_timing.py` already records
...
2026-08-18](DUAL_PORT_STRIPING_PROBE_20260818.md) illustrates the size of the
...
[Dual-port striping and graph-kernel probe record,
2026-08-18](DUAL_PORT_STRIPING_PROBE_20260818.md) records approximately 2%
...
| End-to-end serving quantities | 10% of the compared median | Below the 22-40% C8 same-configuration ranges only because C8 is excluded from attribution claims outright; at the C1 concurrency this design uses, it is close to twice the -5.34% same-configuration C1 within-session drift recorded in [EXL3 performance campaign, 2026-08-02](EXL3_AB_CAMPAIGN_20260802.md), and above that document's largest such drift at any concurrency (+7.45% at C2) |
...
`scripts/bench/collective_attribution.py` (safety class OFFLINE) reads a
...
python scripts/bench/collective_attribution.py --print-schema

# Narrow the interval from two arms, optionally scaled by measured exposure.
python scripts/bench/collective_attribution.py \

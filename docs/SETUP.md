...
**Expected on GB10 — not a bug you can fix.** GPUDirect RDMA registration of `cudaMalloc` and `cudaMallocManaged` pointers fails on both Sparks (documented in `sparkring-next-tests.md` and `SIRCL-DESIGN.md` — both private archive, not in this snapshot). **The supported path is mapped host memory:** `cudaHostAlloc(cudaHostAllocMapped)` arenas registered with `ibv_reg_mr` (measured ~4.5-4.8 us GPU-to-GPU one-way; on this unified-memory part the "host" allocation is the same physical memory). The SparkRing transport does this by design; do not attempt to force a GDR path, and leave `NCCL_CUMEM_ENABLE=0` as specified.

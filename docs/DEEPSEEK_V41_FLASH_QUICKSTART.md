...
| Request limit / sequences / scheduler tokens | 300,000 / 8 / 8,192 |
| `--gpu-memory-utilization` | 0.80 → KV 8.39 GiB = 1,171,588 tokens (3.91× 300K) measured |
| Speculation | DSpark k=5, probabilistic draft, block rejection, adaptive verification off |
...
GPU KV cache size: 1,171,588 tokens, Maximum concurrency for 300,000 tokens per request: 3.91x
...
`--kv-cache-memory-bytes` is not used here; `--gpu-memory-utilization 0.80` sizes the pool
against ~113 GiB free at startup. On the recorded boot the pool came out at 8.39 GiB
(1,171,588 tokens); the engine's own log names ~12 GiB as available at full utilization, so
there is headroom to raise it once the memory picture on your ranks is measured. Keep
`--max-num-seqs 8`: with DSpark k=5 every decode batch is a multiple of 5 or 6 tokens and
the graph capture list follows from it; others report aggregate throughput falling from
C6 to C8. 1M context has not been run on this profile.

## Measured

See the [benchmark record](../performance/records/deepseek-v41-flash/cycle-tp4-dspark5-graphs-20260910.md).
Headline on the recorded cycle (temperature 0, thinking off, eight-category mean, counting
excluded): per-stream decode 56.2 tok/s at C1 and 31.4 at C6; aggregate 49.8 at C1 rising to
159.9 at C6; cold prefill 1,376–1,574 tok/s from 3K to 93K prompt tokens; needle recall at
131K and 262K; vision and tool calling 7/7; 20-minute c=8 soak with zero failures or hangs.

## What is and is not covered

The switchless cycle is at parity with a switched four-Spark fabric on decode-shaped
collectives (88 all-reduces per step cost about 5 ms) and faster on large messages. The
profile is text+vision serving with DSpark; SparkCache, prefill/decode disaggregation and
SIRCL are not part of it. Adaptive verification stays off until flashinfer #5015 is
resolved. Output quality beyond needle recall and the end-to-end checks was not evaluated.

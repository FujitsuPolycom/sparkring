...
| `target_model_variant` | Optional. `nvfp4-spark` (default; the qualified target every receipt describes) or `nvidia-nvfp4` (`nvidia/GLM-5.3-Flash-NVFP4` @ `423acf37583782c51c142d145aef733d72943d93`, NVIDIA's official ModelOpt NVFP4 export: plain `modelopt` quantization, BF16 attention and shared experts, NVFP4 routed experts and dense MLPs; the launcher derives an `--hf-overrides` entry that keeps the BF16 native-MTP layer unquantized). `model_roots` must point at that checkpoint; the launcher verifies its `config.json` and index identities. The cache namespace gains a `-<variant>` suffix so the two checkpoints never share persisted pages, and the render selects `LOAD_FORMAT=safetensors` with a 1,500 s readiness deadline (the ~6 GB shards overflow unified memory under fastsafetensors; the host-mmap load takes ~8 min for 50 GiB per rank, ~12.5 min to ready). Measured 2026-09-10 on four GB10s: 50.1 GiB weights per rank, KLD vs BF16 0.0508 vs 0.0522 for the qualified target on the same panel, decode within 3–10 %, prefill 4–8 % faster. Research-only: no published receipt covers it |

...
| [LMCache with DeepSeek record](LMCACHE_DEEPSEEK_20260819.md) | Records that the cache tier stores and restores this model's key-value state across a full teardown, and the engine scheduler defect that blocks serving from it; includes the attribution caution that a same-process replay does not evidence external caching. |

...
| [Collective critical-path attribution](COLLECTIVE_CRITICAL_PATH_MEASUREMENT.md) | Defines the arms, instance granularity, repetition counts, and detection thresholds required to narrow a collective's critical-path contribution from a residency ceiling to an attribution, and states which part of that interval the design leaves unmeasured. |
...
| [LMCache with DeepSeek-V4-Flash-0731](LMCACHE_DEEPSEEK_20260819.md) | Records external key-value reuse for this checkpoint as `unsupported`: the installed cache package stores and restores its state across a full teardown but every device-side retrieve aborts on a buffer-size mismatch, with the cause not yet isolated to a package, a configuration, or a code path. States the engine requirements that hold regardless, the validation method an external tier must satisfy, and the attribution caution that a same-process replay does not evidence external caching. |

...
fail-closed profile-generation foundation. The complete public composition for
the operator's accepted 262K dynamic-NVFP4, full-CKV-gather, tiered-SIRCL,
target-only exact-Q40 profile is in
[`EXL3_R7_OPERATOR_REPRODUCTION.md`](EXL3_R7_OPERATOR_REPRODUCTION.md).
The remaining runtime artifacts listed under **Limitations** still block a
zero-context clean-checkout live deployment. Do not describe the foundation
profile generated by this document as the accepted operator default.
...
The complete operator-profile generators, SIRCL source, and exact-Q40 overlays
are public, but a clean checkout does not yet contain every prerequisite
artifact in this table. Therefore a reader without independently audited
copies can generate and inspect the exact composition but **cannot truthfully
start it yet**. Do not bypass the hashes or delete the mounts. Publishing the
three missing source/build chains is the remaining public-bootstrap blocker.

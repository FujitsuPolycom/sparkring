...
and aborts if a rank fails to come up. Containers are removed with
`docker rm -f`; there is no persistent service state, so re-running `start`
after a failed `stop` or a host reboot restores the cluster. This controller
only orchestrates the existing per-rank launcher and does not change the
launch contract.

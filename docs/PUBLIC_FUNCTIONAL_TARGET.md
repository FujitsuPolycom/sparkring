...
| TBD-8 | Whether a public-lane image (pinned vLLM commit + only the two published SparkCache patches) can serve with `--attention-backend B12X_MLA_SPARSE` and `--kv-cache-dtype nvfp4_ds_mla` at all — RUNTIME_GAPS.md records the SM121 sparse-MLA backend and low-bit MLA KV record formats as unmerged upstream and supplied by the reference overlay | If it cannot, §2.4 needs a documented substitution before any result is reportable | runtime overlay / provenance workstream |

...
def _main(argv: Sequence[str] | None = None) -> int:
    ...
    if raw and raw[0] == "checkpoints":
        from runtime.host.checkpoints import main as checkpoints_main
        return checkpoints_main(raw[1:])
    if raw and raw[0] == "hairpin":
        from runtime.host.hairpin_ring import main as hairpin_main
        return hairpin_main(raw[1:])
    ...
    if raw and raw[0] == "validate-compose":
        ...
        if root not in sys.path:
            sys.path.insert(0, root)
        from runtime.common.compose_validation import main as validate_compose
        return validate_compose(raw[1:])

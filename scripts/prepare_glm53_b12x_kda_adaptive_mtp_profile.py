...
def _argument(profile: dict[str, Any], option: str) -> str:
    ...
    if not isinstance(arguments, list) or any(not isinstance(value, str) for value in arguments):
        raise ResolveError("extra_vllm_args must be a list of strings")
...
def resolve(
    ...
    *,
    ...
    for owner, fields in ((profile, ("identity", "required_image_labels", "environment")),
    ...
    if not isinstance(speculative, dict):
        raise ResolveError("--speculative-config must be an object")

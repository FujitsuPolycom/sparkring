...
## GLM-5.2 EXL3 3.5-bpw four-Spark serving regression control
...
## Native transport probes at additional payload sizes
...
and baked-environment unset list must be replicated when bypassing it; follower
ranks require `--headless`; the deployed bind-mount set is load-bearing; and
the CuTe-DSL warmup path must remain disabled for the measured image.

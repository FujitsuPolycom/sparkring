...
def test_quickstart_documents_exact_pr25_compatibility_boundary() -> None:
    quickstart = QUICKSTART_PATH.read_text(encoding="utf-8")
    compact = " ".join(quickstart.split())
    assert "## Launch the recorded image with SparkCache pull request 25 keys" in quickstart
    assert "sha256:9faa36a9f37aee16d97ab9214ef3153b4d200121126e6b2dee5ebb63109fea18" in quickstart
    assert "spark_cache_cuda_restore" in quickstart
    assert "spark_cache_native_restore" in quickstart
    assert "spark_cache_cuda_restore_io_workers" in quickstart
    assert "spark_cache_native_io_workers" in quickstart
    assert "pull request #137" in quickstart
    assert "image specified by pull request #137 has not been built or live-validated" in compact

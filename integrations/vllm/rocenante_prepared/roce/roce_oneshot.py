...
from ._path_config import (
    ...
    validate_four_path_mapping as _validate_four_path_mapping,
)
...
class RoceOneshotAllReduce:
    ...
    def __init__(
        ...
        *,
        ...
        self._poison_address = self._epoch_address + 4 * (
            1 + 2 * self._counter_classes
        )
        ...
        config = {
            ...
            "opposite_paths": self._opposite_paths,
            "origin_paths_per_rank": (
                self._opposite_paths
                if self.world_size == 2
                else 2 * (self.world_size - 2) + self._opposite_paths
    ...
    def _counter_addresses(self, blocks: int) -> tuple[int, int]:
        """Stage and tail counters whose modulus matches ``blocks``."""
    ...
    def _aligned_scratch(self, which: int, like: torch.Tensor) -> torch.Tensor:
        ...
        Allocated during preparation (two ``max_size`` buffers); a first use under
        ...
        if self._align_buffers is None:
            if torch.cuda.is_current_stream_capturing():
                raise RuntimeError(
                    ...
                    "capture; prepare the RoCE plan first"
                )
            self._align_buffers = (
        ...
        with self._lock:
            ...
            with torch.cuda.device(self.device), context:
                ...
                if direct:
                    ...
                    self._order_stream(capturing)
                    self._launch_gather(prepared.gather_launcher, inp.data_ptr(), out.data_ptr(), nbytes, row_packs)
                    self._mark_stream(capturing)
                    return out
                ...
                self._launch_gather(
                    prepared.gather_launcher, staged.data_ptr(), gathered.data_ptr(), padded, padded // PACK_BYTES
                )
                stacked = (
                ...
                if out is None:
                    # A pack-sized unaligned shard can produce a contiguous
                    # view of shared gather scratch. Return independent storage
                    # before another collective may reuse that scratch.
    ...
    def _gather_scratch(self, padded: int) -> tuple[torch.Tensor, torch.Tensor]:
        ...
        Plan materialization or the first eager padded gather
    ...
    def _launch_gather(self, launcher, input_address: int, output_address: int, nbytes: int, row_packs: int) -> None:

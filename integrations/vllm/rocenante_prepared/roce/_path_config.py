...
PAIR_PATHS_ENV = "B12X_ROCE_PAIR_PATHS"
CANONICAL_HCAS = (
    "rocep1s0f0",
    "rocep1s0f1",
    "roceP2p1s0f0",
...
CANONICAL_TP4_HCAS = CANONICAL_HCAS
CANONICAL_TP2_FOUR_PATH_MAP = (
    ((-1, -1), (0, 1, 2, 3)),
    ((0, 1, 2, 3), (-1, -1)),
)
...
def opposite_path_count(world_size: int, hca_count: int) -> int:
    """Resolve the research-only four-path mode for a pair or TP4 opposite peer."""

    if world_size == 2:
        raw = os.getenv(PAIR_PATHS_ENV, "").strip()
        if not raw:
            return PATH_COUNT
        try:
            count = int(raw)
        except ValueError as exc:
            raise ValueError(f"{PAIR_PATHS_ENV} must be 2 or 4") from exc
        if count not in (PATH_COUNT, MAX_PATH_COUNT):
            raise ValueError(f"{PAIR_PATHS_ENV} must be 2 or 4")
        if count == MAX_PATH_COUNT and hca_count != MAX_PATH_COUNT:
            raise RuntimeError(
                f"{PAIR_PATHS_ENV}=4 requires four active RDMA devices"
            )
        return count
...
def validate_four_path_mapping(
    hca_names: Sequence[str],
    world_size: int,
    rank: int,
    mapping: tuple[tuple[int, ...], ...],
    ...
    """Require a measured reciprocal mapping before enabling four paths."""

    if path_count != MAX_PATH_COUNT:
        return
    if world_size == 2:
        valid = (
            tuple(hca_names) == CANONICAL_HCAS
            and rank in (0, 1)
            and mapping == CANONICAL_TP2_FOUR_PATH_MAP[rank]
        )
        if not valid:
            raise RuntimeError(
                f"{PAIR_PATHS_ENV}=4 requires the canonical two-rank HCA order "
                "and reciprocal peer-path mapping"
            )
        return
    validate_four_path_tp4_mapping(
        hca_names, world_size, rank, mapping, path_count
    )

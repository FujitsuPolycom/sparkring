...
PREPARED_PROFILE = "tp2-rocenante-adaptive-prepared"
PROFILES = frozenset((PROFILE, PREPARED_PROFILE))
...
HOST_SOURCE_PREFIX = "/opt/venv/lib/python3.12/site-packages/b12x/"
HOST_SOURCE_FILES = frozenset((
    "preparation/__init__.py", "preparation/types.py", "preparation/tuning.py",
    "preparation/session.py", "_lib/compile_plan.py", "_lib/program_cache.py",
))


def _verify_host_sources(preimages: dict[str, str]) -> None:
    """Refuse a prepared bundle built for another B12X declaration/runtime ABI."""
    if not isinstance(preimages, dict) or set(preimages) != {
        HOST_SOURCE_PREFIX + name for name in HOST_SOURCE_FILES
    }:
        raise ValueError("Prepared transport requires the complete B12X source preimages")
    for name, expected in preimages.items():
        path = Path(name)
        if not re.fullmatch(r"[0-9a-f]{64}", expected):
            raise ValueError("Invalid B12X source preimage hash")
        if path.is_symlink() or hashlib.sha256(path.read_bytes()).hexdigest() != expected:
            raise ValueError("Prepared transport B12X source differs: " + name)


def _host_uses_prepared_api() -> bool:
    return Path(HOST_SOURCE_PREFIX + "preparation/__init__.py").is_file()
...
def verify_bundle(name: str, digest: str, root: Path = ROOT) -> Path:
    ...
    if name == PROFILE and _host_uses_prepared_api():
        raise ValueError("Prepared B12X requires transport profile " + PREPARED_PROFILE)
    ...
    if name == PREPARED_PROFILE:
        _verify_host_sources(manifest.get("image_source_preimages"))
    return bundle / "roce"

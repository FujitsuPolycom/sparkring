...
- A width outside `[1, 1048576]` is rejected. The upper bound keeps
  `1048576 elements x 512 rows x 2 bytes = 1 GiB` under the native
  single-RDMA-write bound of `UINT32_MAX` bytes.
- Port slots follow two regimes. A payload of the form `rows * 6144 * 2` for a
  row in the resolved set takes slot `row - 1`, so a row the policy omits
  leaves a permanent hole in the sequence rather than shifting later rows. A
  payload generated by an admitted non-default width takes a slot based at
  512, past the largest legacy span, ordered by its position in the sorted
  extension set. The base is a constant, so no row policy and no
  `VLLM_SPARK_TP4_PREFILL_Q512` toggle moves an extension port. A non-default
  width whose byte count equals a legacy payload resolves to the legacy
  slot.

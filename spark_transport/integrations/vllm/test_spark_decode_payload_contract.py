...
import spark_decode_payload_contract as contract
...
def test_checked_in_deliverable_is_generated_from_contract() -> None:
    repository = Path(__file__).resolve().parents[3]
    deliverable = (
        repository
        / "deliverables"
        / "glm52-decode-payload-contract-c1-c8.md"
    )
    try:
        checked_in = deliverable.read_text(encoding="utf-8")
    except FileNotFoundError:
        pytest.skip("deliverable lives in the private archive")
    assert checked_in == contract.render_markdown()

...
int main() {
  ...
  assert(bind(reservation, reinterpret_cast<sockaddr*>(&address), sizeof(address)) == 0);
  socklen_t size = sizeof(address);
  assert(getsockname(reservation, reinterpret_cast<sockaddr*>(&address), &size) == 0);
  const auto port = ntohs(address.sin_port);
  auto client = std::async(std::launch::async, [port] {
    auto channel = spark_transport::ControlChannel::connect("127.0.0.1", port);
    assert(channel.exchange<std::uint32_t>(17) == 23);
  });
  std::this_thread::sleep_for(std::chrono::milliseconds(150));
  close(reservation);
  {
    auto server = spark_transport::ControlChannel::listen_and_accept(port);
    assert(server.exchange<std::uint32_t>(23) == 17);
    client.get();
  }
  ...
  assert(elapsed < std::chrono::seconds(12));
}

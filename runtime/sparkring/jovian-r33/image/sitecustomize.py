"""Compose the embedded SIRCL hooks with the virtual-diagonal adapter.

Status: implemented. The R33 image installs this file as the process
``sitecustomize`` and preserves the SIRCL entry point beside it as
...
BASE_SITECUSTOMIZE = Path(__file__).resolve().with_name("sircl_sitecustomize.py")
...
def _required(label: str, operation) -> None:
    try:
        ...
        try:
            print(
                f"FATAL: required startup hook failed: {label}",
                file=sys.stderr,
                flush=True,
            )

...
Each power-of-two grid size has separate staging and tail counters. Message
size and launch grid are runtime scalars, so small collectives avoid the full
grid without allowing one launch to corrupt another grid's arrival state.
"""
...
class _RoceOneshotLaunch:
    def __init__(
        ...
        if self._opposite_paths == 4 and self._world_size not in (2, 4):
            raise ValueError("four paths require a two- or four-rank group")
            ...
            if cutlass.const_expr(self._opposite_paths == 4):
                ...
                for peer in cutlass.range_constexpr(self._world_size):
                    if cutlass.const_expr(peer != self._rank):
                        four_path_peer = self._world_size == 2 or (
                            peer - self._rank
                        ) % self._world_size == 2
                        peer_paths = 4 if four_path_peer else 2

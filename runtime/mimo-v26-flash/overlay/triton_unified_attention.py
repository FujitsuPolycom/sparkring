...
def unified_attention(
    ...
    use_3d = not (
        ...
        # Partial outputs and reduction scalars are indexed by query token,
        # not by sequence, so short multi-token speculative-verification
        # batches can take the split-KV path when every query token fits the
        # workspace; larger prefills keep the 2D path. Same dispatch rule as
        # local-inference-lab/vllm PR 839 for the DiffKV kernel.

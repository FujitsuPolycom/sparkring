...
class MiMoVisionAttention(nn.Module):
    def __init__(
        ...
        # Data-parallel encoder mode keeps every head on every rank, so the
        # per-head sink slice in the window-attention path must start at
        # head 0 on every rank; only weight-sharded mode offsets by rank.
        self.tp_rank = (
            0
            if use_data_parallel
...
class MiMoV2OmniForCausalLM(
    ...
    def get_mm_mapping(self) -> MultiModelKeys:
        """Module prefixes of the language model, the connector and the towers."""
    ...
    def get_mm_lora_token_counts(
        ...
        *,
        ...
        """Rows entering the tower and the connector for one multimodal item.

        ``num_mm_embeds`` is the item's placeholder count in the prompt. Each
        vision placeholder is one ``spatial_merge_size**2`` group of ViT patch
        rows (temporal patching is already folded into the patch rows), and
        ``visual.merger`` consumes one flattened group per placeholder. Audio
        placeholders map one-to-one onto the audio encoder's grouped frames,
        and the audio path has no separate connector (its projection is part
        of the tower), so its connector count is ``None``.

        The b12x eager warmup (``vllm/model_executor/warmup/b12x_prepare.py``,
        ``mark_b12x_eager_shapes``) calls this with ``modality="image"`` and
        ``num_mm_embeds`` equal to the encoder budget and requires a positive
        tower count; the ``SupportsMultiModal`` default returns ``None``.
        """
        del mm_kwargs
        if modality in ("image", "video"):
            merge_unit = self.visual.spatial_merge_unit
            return num_mm_embeds * merge_unit, num_mm_embeds
        if modality == "audio":

...
# Opt-in attention geometry switches (default off leaves behaviour unchanged):
#
# VLLM_MIMO_PAD_V=1
#   MiMo-V2 attention has head_dim=192 for Q/K and v_head_dim=128 for V. The
#   default path selects a DiffKV attention backend (FLASH_ATTN_DIFFKV or
#   TRITON_ATTN_DIFFKV) whose Triton implementation rejects quantized KV
#   caches. With this flag the layer zero-pads V to head_dim before the
#   attention call, declares a symmetric (192, 192) geometry to ``Attention``
#   so the configured standard backend (``--attention-backend``, e.g.
#   TRITON_ATTN) serves the layer, and slices the attention output back to
#   v_head_dim before o_proj. Numerically identical to the unpadded path: the
#   padded V columns are zero, so their output columns are zero and are
#   discarded. Cost: the V half of every KV page grows by head_dim/v_head_dim
#   (1.5x for MiMo-V2); the K half is unchanged. Ignored when
#   ``--attention-backend`` names a *_DIFFKV backend.
#
# VLLM_MIMO_PASS_CACHE_CONFIG=1
#   ``MiMoV2FlashDecoderLayer`` does not forward ``vllm_config.cache_config``
#   to ``MiMoV2Attention``, so every MiMo attention layer is built with
#   ``cache_config=None`` and ``Attention`` falls back to kv_cache_dtype
#   "auto" (the model dtype) regardless of ``--kv-cache-dtype``. With this
#   flag the decoder layer forwards the cache config, so ``--kv-cache-dtype``
#   and ``--kv-cache-dtype-skip-layers`` apply to the target's attention
#   layers. Combined with the default DiffKV path and a quantized KV dtype the
#   Triton DiffKV implementation raises NotImplementedError at start-up; use it
#   together with VLLM_MIMO_PAD_V=1 (or a bf16 KV cache).
_MIMO_PAD_V_ENV = "VLLM_MIMO_PAD_V"
_MIMO_PASS_CACHE_CONFIG_ENV = "VLLM_MIMO_PASS_CACHE_CONFIG"
...
class MiMoV2Attention(nn.Module):
    def __init__(
        ...
        requested_is_diffkv = requested is not None and requested.name.endswith(
            "_DIFFKV"
        )

        # Padded-V geometry (see the VLLM_MIMO_PAD_V note at module top): V is
        # zero-padded from v_head_dim to head_dim so a symmetric backend serves
        # the layer; the attention output is sliced back before o_proj.
        self.pad_value_for_attn = (
            self.v_head_dim != self.head_dim
            and _env_flag(_MIMO_PAD_V_ENV)
            and not requested_is_diffkv
        )
        if self.v_head_dim != self.head_dim and _env_flag(_MIMO_PAD_V_ENV):
            if requested_is_diffkv:
                logger.warning_once(
                    "%s=1 ignored because --attention-backend is %s.",
                    _MIMO_PAD_V_ENV,
                    requested.name,
                )
            else:
                logger.info_once(
                    "%s=1: padding V from %d to %d and using the configured "
                    "symmetric attention backend (%s) instead of a DiffKV backend.",
                    _MIMO_PAD_V_ENV,
                    self.v_head_dim,
                    self.head_dim,
                    requested.name if requested is not None else "auto",
                )
        attn_head_size_v = self.head_dim if self.pad_value_for_attn else self.v_head_dim
        ...
        if self.v_head_dim != self.head_dim and not self.pad_value_for_attn:
            if requested_is_diffkv:
    ...
    def forward(
        ...
        if self.pad_value_for_attn:
            # [T, nkv * v_head_dim] -> [T, nkv * head_dim], zero-filled tail.
        ...
        if self.pad_value_for_attn:
            # [T, nh * head_dim] -> [T, nh * v_head_dim]; the dropped columns
            # are exact zeros (attention over zero-padded V).
...
class MiMoV2FlashDecoderLayer(nn.Module):
    def __init__(self, vllm_config: VllmConfig, prefix: str = "") -> None:
        ...
        # See the VLLM_MIMO_PASS_CACHE_CONFIG note at module top. Default None
        # keeps the attention layers on kv_cache_dtype "auto".
        attn_cache_config = (
            vllm_config.cache_config
            if _env_flag(_MIMO_PASS_CACHE_CONFIG_ENV)
            else None
        )

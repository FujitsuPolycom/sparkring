...
def find_next_prime(start, seen):
    c = start + 1
    while not _is_prime(c) or c in seen: c += 1
    return c

def head_sizes_per_layer(cfg):
    layers = list(cfg["engram_layer_ids"]); seen = set(); out = []
    for _ in layers:
        flat = []
        for _ in range(cfg["engram_max_ngram_size"] - 1):
            cur = cfg["engram_vocab_size"] - 1
            for _ in range(cfg["engram_n_heads"]):
                cur = find_next_prime(cur, seen); seen.add(cur); flat.append(cur)
        out.append(flat)
    return layers, out
...
def tensor_loc(model_dir, name):
    ...
    m = hdr[name]; return path, 8 + n + m["data_offsets"][0], tuple(m["shape"]), m["dtype"]

def main():
    ap = argparse.ArgumentParser()
    ap.add_argument("--model-dir", required=True); ap.add_argument("--out-dir", required=True)
    ap.add_argument("--tp", type=int, required=True); ap.add_argument("--rank", type=int, required=True)
    ...
    for layer, hs in zip(layers, sizes):
        ...
        assert wdt == "F8_E4M3" and sdt == "F8_E8M0" and wshape[0] == sshape[0], (wdt, sdt, wshape, sshape)
        rows, dim, sb = wshape[0], wshape[1], sshape[1]; rb = dim + sb
        assert sum(hs) <= rows
        cum = [0]
        ...
        for lo, hi in ranges:
            for s in range(lo, hi, a.chunk_rows):
                ...
                w = np.frombuffer(os.pread(wfd, n * dim, woff + s * dim), dtype=np.uint8).reshape(n, dim)
                sc = np.frombuffer(os.pread(sfd, n * sb, soff + s * sb), dtype=np.uint8).reshape(n, sb)
                buf = np.empty((n, rb), dtype=np.uint8); buf[:, :dim] = w; buf[:, dim:] = sc
                off = s * rb; mv = memoryview(buf).cast("B"); written = 0
                while written < len(mv):
                    written += os.pwrite(ofd, mv[written:], off + written)
                done += n
            print(f"  range [{lo}, {hi}) done ({done:,} rows, {time.time()-t0:.0f} s)", flush=True)
        os.fsync(ofd); os.close(ofd); os.close(wfd); os.close(sfd)
        os.replace(tmp, out)
        json.dump({"layer": layer, "rows": rows, "row_bytes": rb, "dim": dim, "sb": sb, "tp": a.tp, "rank": a.rank,
                   "mode": "balanced" if a.balanced else "contiguous", "cols": cols, "ranges": ranges,
                   "source": os.path.basename(wpath), "built": time.strftime("%Y-%m-%dT%H:%M:%SZ", time.gmtime())},
                  open(mf, "w"), indent=1)

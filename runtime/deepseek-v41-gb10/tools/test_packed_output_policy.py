...
def load_packer():
    spec = importlib.util.spec_from_file_location('synthetic_engram_packer', Path(__file__).with_name('pack_engram_rows.py'))
...
def arguments(module, monkeypatch, model, output):
    monkeypatch.setattr(module.argparse.ArgumentParser, 'parse_args', lambda self: SimpleNamespace(
        model_dir=str(model), out_dir=str(output), tp=1, rank=0, balanced=True, contiguous=False, chunk_rows=2))


@pytest.mark.parametrize('kind', ['binary', 'manifest', 'partial', 'empty'])
def test_existing_destination_is_preserved_before_source_reads(tmp_path, monkeypatch, kind):
    module = load_packer()
...
def test_fresh_destination_packs_tiny_synthetic_rows(tmp_path, monkeypatch):
    ...
    source.mkdir()
    (source/'config.json').write_text(json.dumps({'engram_max_ngram_size': 2, 'engram_n_heads': 1}))
    ...
    monkeypatch.setattr(module, 'tensor_loc', lambda root, name: (str(weights), 0, (2, 2), 'F8_E4M3') if name.endswith('weight') else (str(scales), 0, (2, 1), 'F8_E8M0'))
    ...
    assert json.loads((output/'engram-l1-packed.bin.json').read_text())['ranges'] == [[0, 2]]

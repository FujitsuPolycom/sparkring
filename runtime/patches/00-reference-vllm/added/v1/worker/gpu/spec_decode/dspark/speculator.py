...
class DSparkSpeculator(DraftModelSpeculator):
    def __init__(self, vllm_config, device: torch.device):
        ...
        # PP supported: the speculator lives on the last stage, which owns
        # the aux capture layers, lm_head, and (via loader) an embed copy.
        ...
        # STEP 2: confidence-scheduled adaptive draft length (opt-in).
        # Default "off" keeps the full fixed draft block exactly as before.
        # The captured CUDA-graph forward shape is never changed; we keep
        # running the full fixed block and only truncate the *emitted* draft
        # token ids per request by setting trailing positions to -1.
        self._confidence_scheduler = self._read_confidence_scheduler()
        self._confidence_threshold = self._read_confidence_threshold()
        self._confidence_truncation_enabled = (
            self._confidence_scheduler == "threshold"
        )
        # Persistent confidence buffer so the captured FULL graph can surface
        # the (already-computed) confidence for post-graph truncation. Only
        # allocated when truncation is enabled, so the default path is
        # byte-for-byte unchanged (no extra graph copy is captured).
        self.draft_confidence: torch.Tensor | None = None
        if self._confidence_truncation_enabled:
            ...
            logger.info(
                "DSpark confidence-scheduled draft length enabled "
                "(mode=threshold, threshold=%.4f). The fixed draft block "
                "still runs; emitted draft tokens are truncated per request "
                "with trailing positions set to -1.",
                self._confidence_threshold,
            )

    @staticmethod
    def _read_confidence_scheduler() -> str:
        raw = os.getenv("VLLM_DSPARK_CONFIDENCE_SCHEDULER", "off").strip().lower()
        if raw in {"", "off"}:
            return "off"
        if raw == "threshold":
            return "threshold"
        if raw == "hardware":
            logger.warning(
                "VLLM_DSPARK_CONFIDENCE_SCHEDULER='hardware' is not supported "
                "by the DSpark block speculator (no profiled steps-per-second "
                "curve is wired into this runtime); falling back to 'off' "
                "(full draft block)."
            )
            return "off"
        raise ValueError(
            "VLLM_DSPARK_CONFIDENCE_SCHEDULER must be one of 'off', "
            f"'threshold', 'hardware', got {raw!r}"
        )
    ...
    def _truncate_draft_by_confidence(
        self,
        num_reqs: int,
        confidence: torch.Tensor,
    ) -> None:
        """Shorten emitted draft tokens per request from DSpark confidence.

        The fixed-size draft block still runs unchanged; this only truncates
        the *emitted* token ids by setting trailing positions to -1, mirroring
        the reference confidence-threshold schedule
        (``confidence_threshold_prefix_length``). ``confidence`` holds the
        draft head's raw acceptance logits, so cumulative survival uses their
        sigmoid. The decision is non-anticipating: because cumulative survival
        is non-increasing, the admitted length is the count of leading
        positions whose cumulative survival stays >= threshold.
        """
        if num_reqs <= 0:
            return
        ...
        if steps <= 0:
            return
        probs = confidence[:num_reqs, :steps].float().sigmoid()
        survival = torch.cumprod(probs, dim=1)
        keep = survival >= self._confidence_threshold
        lengths = keep.to(torch.int64).sum(dim=1, keepdim=True)
        cols = self.draft_step_cols[:steps].view(1, steps)
        drop_mask = cols >= lengths
        self.draft_tokens[:num_reqs, :steps].masked_fill_(drop_mask, -1)
    ...
    def _generate_draft_graph(
        ...
        with set_forward_context(
            ...
            output_ids, _, confidence = self.model.forward_spec(
        ...
        # STEP 2: when confidence truncation is enabled, capture the (already
        # computed) confidence into a persistent buffer so the FULL-graph
        # replay can be truncated after run_fullgraph. Gated, so the default
        # captured graph is unchanged.
        if self._confidence_truncation_enabled and self.draft_confidence is not None:
            self.draft_confidence[:num_reqs].copy_(confidence[:, :steps])
    ...
    def propose(
        ...
        if batch_desc is not None and batch_desc.cg_mode == CUDAGraphMode.FULL:
            ...
            if (
                self._confidence_truncation_enabled
                and self.draft_confidence is not None
            ):
                self._truncate_draft_by_confidence(num_reqs, self.draft_confidence)
            return self.draft_tokens[:num_reqs]
        ...
        with record_function_or_nullcontext("vllm:v2/speculator/dspark/forward"):
            ...
            with set_forward_context(
                ...
                output_ids, logits, confidence = self.model.forward_spec(
                    input_ids,
                    main_hidden,
                    positions,
                    idx_mapping=self.idx_mapping[:num_reqs],
        ...
        steps = self.num_speculative_steps
        self.draft_tokens[:num_reqs, :steps].copy_(output_ids[:, 1 : 1 + steps])
        # STEP 2: truncate emitted draft tokens by confidence (opt-in). Default
        # "off" leaves the full block exactly as before.
        if self._confidence_truncation_enabled:
            self._truncate_draft_by_confidence(num_reqs, confidence)
        return self.draft_tokens[:num_reqs]

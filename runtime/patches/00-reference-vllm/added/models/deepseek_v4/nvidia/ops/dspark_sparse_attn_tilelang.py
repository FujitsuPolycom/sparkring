...
def _build_dspark_sparse_attn_kernel(num_heads: int, head_dim: int, scale: float):
    ...
    def kernel(h: int, d: int, softmax_scale: float):
        ...
        num_stages = 2
        threads = 128 if num_heads <= 16 else 256  # PR#65: TP=4 (<=16 heads/rank) needs warp_col_tiles>=8
...
def dspark_sparse_attn(
    ...
    HEAD_CHUNK = 16
    if heads > HEAD_CHUNK:
        # The kernel's dynamic shared memory scales with heads-per-launch;
        # above 16 heads (TP<4, e.g. the PP last-stage draft at TP1/TP2) it
        # exceeds the sm121 per-block limit. Launch per 16-head slice over
        # the shared kv — the exact kernel shape the TP=4 path runs.
        q = q.contiguous()
        kv = kv.contiguous()
        kernel = _build_dspark_sparse_attn_kernel(
            HEAD_CHUNK, head_dim, softmax_scale
        )
        outs = []
        for h0 in range(0, heads, HEAD_CHUNK):
            h1 = min(h0 + HEAD_CHUNK, heads)
            qc = q[:, :, h0:h1].contiguous()
            sc = attn_sink[h0:h1].contiguous().float()
            if h1 - h0 < HEAD_CHUNK:
                pad = HEAD_CHUNK - (h1 - h0)
                qc = torch.cat(
                    [qc, qc.new_zeros(batch, draft_tokens, pad, head_dim)],
                    dim=2,
                )
                sc = torch.cat([sc, sc.new_zeros(pad)])
            oc = torch.empty_like(qc)
            kernel(qc, kv, oc, sc, topk_idxs)
            outs.append(oc[:, :, : h1 - h0])

...
class GPUModelRunner(
    ...
    def _build_attention_metadata(
        ...
        checkpoint_plans_cpu = None
        if recurrent_prefill_checkpoint_plans:
            if for_cudagraph_capture or ubatch_slices is not None:
                raise ValueError("Planned recurrent checkpoints require unsliced eager metadata")
            checkpoint_plans_cpu = [None] * num_reqs_padded
            for req_id, plan in recurrent_prefill_checkpoint_plans.items():
                row = self.input_batch.req_id_to_index.get(req_id)
                if row is None or not 0 <= row < num_reqs:
                    raise ValueError("Checkpoint plan request is absent from the active batch")
                start, end, targets = plan
                if (
                    start < 0 or end <= start or len(targets) > 2
                    or tuple(sorted(set(targets))) != targets
                    or any(not start < target < end for target in targets)
                    or num_scheduled_tokens is None
                    or num_scheduled_tokens.get(req_id) != end - start
                ):
                    raise ValueError("Checkpoint plan does not match the scheduled query")
                checkpoint_plans_cpu[row] = plan
        ...
        with (
            ...
            attn_metadata, spec_decode_common_attn_metadata = (
                self._build_attention_metadata(
                    ...
                    recurrent_prefill_checkpoint_plans=getattr(
                        scheduler_output, "recurrent_prefill_checkpoint_plans", None
                    ),

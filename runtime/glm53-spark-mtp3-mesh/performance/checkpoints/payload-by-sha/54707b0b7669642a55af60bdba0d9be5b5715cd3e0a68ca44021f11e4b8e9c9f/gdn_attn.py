...
class GDNPrefillCheckpointMetadata:
    ...
    # Required planned targets must never use the legacy NULL/disabled export.
    ...
    def build(  # type: ignore[override]
        ...
        if (
            ...
            if plans is not None and len(plans) < len(all_query_lens):
                raise ValueError("checkpoint plan rows do not cover the current batch")
            checkpoint_offsets = []
            checkpoint_columns = []
            checkpoint_required = []
            for row in request_rows:
                ...
                offsets, columns = checkpoint_metadata(
                    None if plans is None else plans[row], seq_len - query_len,
                    seq_len, block_size, capacity)
                checkpoint_offsets.append(offsets if capacity == 2 else offsets[0])
                checkpoint_columns.append(columns if capacity == 2 else columns[0])
                required = [plans is not None and plans[row] is not None and column >= 0
                            for column in columns]
                checkpoint_required.append(required if capacity == 2 else required[0])

            any_checkpoint = (any(any(row) for row in checkpoint_offsets) if capacity == 2
                              else any(checkpoint_offsets))
            if any_checkpoint:
                ...
                checkpoint_state_indices = m.block_table_tensor[
                    request_rows_tensor[:, None] if capacity == 2 else request_rows_tensor, checkpoint_columns_tensor
                ]
                ...
                has_required = (any(any(row) for row in checkpoint_required) if capacity == 2
                                else any(checkpoint_required))
                required_mask = None
                if has_required:
                    required_mask = async_tensor_h2d(
                        checkpoint_required, dtype=torch.bool, device=query_start_loc.device)
                    torch._assert_async(
                        torch.all(~required_mask | (checkpoint_state_indices != NULL_BLOCK_ID)),
    ...
    def update_block_table(
        ...
        if prefill_checkpoint is not None:
            ...
            if prefill_checkpoint.required_mask is not None:
                torch._assert_async(
                    torch.all(~prefill_checkpoint.required_mask | (checkpoint_state_indices != NULL_BLOCK_ID)),
                    "rebound planned recurrent checkpoint refers to a NULL state block")

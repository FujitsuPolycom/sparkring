...
def _checkpoint_plan_rows(input_batch, plans, num_reqs, *, for_capture=False):
    """Map immutable scheduler plans to this actual V2 batch's request order."""
    if not plans:
        return None
    if for_capture:
        raise ValueError("Planned recurrent checkpoints cannot enter graph capture")
    if len(input_batch.req_ids) != input_batch.num_reqs or num_reqs < input_batch.num_reqs:
        raise ValueError("Checkpoint batch request geometry differs")
    rows = [None] * num_reqs
    positions = {request_id: row for row, request_id in enumerate(input_batch.req_ids)}
    if len(positions) != input_batch.num_reqs:
        raise ValueError("Checkpoint batch has duplicate request IDs")
    for request_id, plan in plans.items():
        row = positions.get(request_id)
        if row is None:
            raise ValueError("Checkpoint plan request is absent from the active V2 batch")
        start, end, targets = plan
        if (type(start) is not int or type(end) is not int or start < 0 or end <= start
                or not isinstance(targets, tuple) or not 1 <= len(targets) <= 2
                or any(type(target) is not int for target in targets)
                or targets != tuple(sorted(set(targets)))
                or any(not start < target < end for target in targets)
                or int(input_batch.num_computed_tokens_np[row]) != start
                or int(input_batch.num_scheduled_tokens[row]) != end - start
                or int(input_batch.query_start_loc_np[row + 1] - input_batch.query_start_loc_np[row]) != end - start):
            raise ValueError("Checkpoint plan does not match the actual V2 query span")
...
class MambaHybridAttnMetadata(ModelSpecificAttnMetadata):
    ...
    def get_extra_common_attn_kwargs(
        ...
        values = {"is_prefilling": self.is_prefilling[:num_reqs]}
        if self.recurrent_prefill_checkpoint_plans_cpu is not None:
            values["recurrent_prefill_checkpoint_plans_cpu"] = (
                self.recurrent_prefill_checkpoint_plans_cpu[:num_reqs]
            )
        return values
...
class MambaHybridModelState(DefaultModelState):
    ...
    def prepare_attn(
        ...
        checkpoint_plans_cpu = _checkpoint_plan_rows(
            input_batch, recurrent_prefill_checkpoint_plans, num_reqs,
            for_capture=for_capture,
        )
        mamba_attn_metadata = MambaHybridAttnMetadata(
            recurrent_prefill_checkpoint_plans_cpu=checkpoint_plans_cpu,
        ...
        if checkpoint_plans_cpu is not None:
            checked_layers = 0
            for groups in attn_groups:
                for group in groups:
                    if not isinstance(group.get_metadata_builder(0), GDNAttentionMetadataBuilder):
                        continue
                    for layer_name in group.layer_names:
                        checkpoint = getattr(attn_metadata[layer_name], "prefill_checkpoint", None)
                        if checkpoint is None or checkpoint.required_mask is None:
                            raise RuntimeError("V2 scheduled a checkpoint plan without GDN export metadata")
                        checked_layers += 1
            if not checked_layers:
                raise RuntimeError("V2 scheduled recurrent checkpoints without an active GDN layer")
            if not getattr(self, "_explicit_checkpoint_plan_logged", False):
                logger.info("V2 explicit recurrent checkpoint plan active: rows=%s layers=%d",
                            checkpoint_plans_cpu, checked_layers)
                self._explicit_checkpoint_plan_logged = True
        if self.recoverssm is not None:

...
and optionally checkpoint slots with chunk-aligned token offsets, and the
op reads and writes those slots directly. ``Caps.max_checkpoints`` defaults to
one with legacy vector metadata. Explicit ``max_checkpoints=2`` uses contiguous
``[sequence_capacity, 2]`` checkpoint indices/offsets, requires checkpoint export
and transactional validation, and exports both states during the same recurrence. ``Caps.null_state_index`` may reserve

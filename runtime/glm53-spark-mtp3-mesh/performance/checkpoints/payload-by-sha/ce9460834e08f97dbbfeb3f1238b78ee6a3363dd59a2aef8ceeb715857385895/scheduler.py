...
import time
import os
from vllm.v1.core.recurrent_prefill_checkpoint import fresh_prompt_plan
...
class Scheduler(SchedulerInterface):
    def __init__(
        ...
        self._two_checkpoint_prefill_enabled = (
            os.environ.get("SPARK_GDN_PREFILL_CHECKPOINTS") == "2"
            and glm5_next_mtp_has_independent_draft_state
            and self.mamba_has_prefill_checkpoint_blocks
            and all(not isinstance(group.kv_cache_spec, MambaSpec)
                    or (group.kv_cache_spec.num_prefill_checkpoint_blocks >= 2
                        and group.kv_cache_spec.block_size == self.cache_config.block_size)
                    for group in kv_cache_config.kv_cache_groups)
        )
        # An interior recurrent-page hash needs an explicit tail stop.
    ...
    def _recurrent_checkpoint_plan(self, request, start, end):
        if (not self._two_checkpoint_prefill_enabled or start != 0
                or request.num_computed_tokens != 0 or request.has_encoder_inputs):
            return None
        for manager in self.kv_cache_manager.coordinator.single_type_managers:
            if isinstance(manager.kv_cache_spec, MambaSpec) and (
                ...
                return None
        return fresh_prompt_plan(
            start=start, end=end, prompt=request.num_prompt_tokens,
            num_tokens=request.num_tokens, block_size=self.cache_config.block_size,
            publications=self._recurrent_publication_boundaries(request),
    ...
    def _mamba_block_aligned_split(
        ...
        checkpoint_plan = self._recurrent_checkpoint_plan(request, start, start + num_new_tokens)
        if checkpoint_plan is not None:
            return checkpoint_plan[1] - checkpoint_plan[0]
        ...
        if use_internal_checkpoint and not self.use_eagle:
    ...
    def schedule(self, throttle_prefills: bool = False) -> SchedulerOutput:
            ...
            with record_function_or_nullcontext("schedule: allocate_slots"):
                ...
                while True:
                    new_blocks = self.kv_cache_manager.allocate_slots(
                        ...
                        recurrent_prefill_checkpoint_plan=checkpoint_plan,
                        recurrent_checkpoint_publications=(
                            self._recurrent_publication_boundaries(request) if checkpoint_plan else ()),
                    )

                    if new_blocks is not None:
                        if checkpoint_plan is not None:
                            recurrent_prefill_checkpoint_plans[request.request_id] = checkpoint_plan
        ...
        if not preempted_reqs and self._pause_state == PauseState.UNPAUSED:
            ...
            while (self.waiting or self.skipped_waiting) and token_budget > 0:
                ...
                checkpoint_plan = (None if load_kv_async else self._recurrent_checkpoint_plan(
                    request, num_computed_tokens, num_computed_tokens + num_new_tokens))
                new_blocks = self.kv_cache_manager.allocate_slots(
                    request,
                    num_new_tokens,
        ...
        scheduler_output = SchedulerOutput(
            ...
            recurrent_prefill_checkpoint_plans=(
                {req_id: plan for req_id, plan in recurrent_prefill_checkpoint_plans.items()
                 if req_id in num_scheduled_tokens} or None),

...
class SingleTypeKVCacheManager(ABC):
    ...
    def __init__(
        ...
        # Request-local external publication target, set only around one cache
        # operation. Native prefix-cache hashes retain block_pool.hash_block_size.
        self.recurrent_publication_boundary: int | None = None
        self._planned_recurrent_checkpoints: dict[str, tuple[int, int, tuple[int, ...]]] = {}
        self._planned_recurrent_publications: dict[str, tuple[int, ...]] = {}
        ...
        self._pending_partial_tail_offloads: list[
            tuple[str, int, KVCacheBlock, int, BlockHashWithGroupId]
        ] = []
    ...
    def take_pending_partial_tail_offloads(
        self,
    ) -> list[tuple[str, int, KVCacheBlock, int, BlockHashWithGroupId]]:
        """Drain producer partial-tail hand-offs.

        Entries include request, group, block, boundary, and its exact hash.
    ...
    def cache_blocks(
        ...
        planned = self._planned_recurrent_checkpoints.get(request.request_id)
        if planned is not None and block_mask is not None:
            for position in planned[2]:
                index = position // self.block_size - 1
                if not num_cached_blocks <= index < num_full_blocks:
                    raise ValueError("planned checkpoint outside cache registration span")
                block_mask[index - num_cached_blocks] = True
...
class MambaManager(SingleTypeKVCacheManager):
    ...
    def __init__(
        ...
        if self.mamba_cache_mode == "align":
            ...
            self._producer_partial_tail_reqs: dict[
                str, tuple[int, BlockHashWithGroupId]
            ] = {}
    ...
    def get_num_blocks_to_allocate(
        ...
        assert isinstance(self.kv_cache_spec, MambaSpec)
        planned = self._planned_recurrent_checkpoints.get(request_id)
        if planned is not None:
            if self.req_to_blocks.get(request_id) or new_computed_blocks or total_computed_tokens:
                raise ValueError("checkpoint admission requires a fresh recurrent table")
            return len(planned[2]) + 1 + self.num_speculative_blocks
        if (
            len(new_computed_blocks) > 0
    ...
    def allocate_new_blocks(
        ...
        planned = self._planned_recurrent_checkpoints.get(request_id)
        if planned is not None:
            start, end, targets = planned
            if start != 0 or num_tokens_main_model != end or self.req_to_blocks.get(request_id):
                raise ValueError("checkpoint allocation span changed after admission")
            final_column = cdiv(end, self.block_size) - 1
            columns = [p // self.block_size - 1 for p in targets]
            columns += [final_column]
            columns += list(range(final_column + 1, final_column + 1 + self.num_speculative_blocks))
            if len(set(columns)) != len(columns):
                raise ValueError("checkpoint output aliases another writable state")
            physical = self.block_pool.get_new_blocks(len(columns))
            table = [self._null_block] * (final_column + 1 + self.num_speculative_blocks)
            for column, block in zip(columns, physical):
                table[column] = block
            self.req_to_blocks[request_id].extend(table)
            ...
            if num_required_blocks <= len(req_blocks) and not has_partial_hit:
                ...
                if partial_hit is not None:
                    ...
                    if blocks_allocated:
                        ...
                        boundary_record = self._producer_partial_tail_reqs.pop(
                            request_id, None
                        )
                        if boundary_record is not None:
                            boundary_tokens, boundary_hash = boundary_record
    ...
    def cache_blocks(
        ...
        if self.mamba_cache_mode == "align":
            ...
            for boundary in self._planned_recurrent_publications.get(request.request_id, ()):
                self._queue_aligned_recurrent_boundary(request, boundary, expected_boundary=boundary)
    ...
    def _cache_partial_tail_block(
        ...
        if num_tokens % hash_block_size != 0:
            return None
        publication_boundary = self.recurrent_publication_boundary
        if publication_boundary is None:
            publication_boundary = (
                request.num_prompt_tokens // hash_block_size
            ) * hash_block_size
        if num_tokens != publication_boundary:
            return None
    ...
    def _queue_aligned_recurrent_boundary(
        ...
        if num_tokens <= 0 or num_tokens % self.block_size != 0:
            return
        replay_boundary = expected_boundary if expected_boundary is not None else (
            self.recurrent_publication_boundary
            if self.recurrent_publication_boundary is not None
            else ((request.num_prompt_tokens - 1) // self.block_pool.hash_block_size)

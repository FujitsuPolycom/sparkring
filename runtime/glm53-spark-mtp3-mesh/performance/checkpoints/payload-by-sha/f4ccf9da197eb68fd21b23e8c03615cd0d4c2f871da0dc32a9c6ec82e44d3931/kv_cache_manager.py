...
class KVCacheManager:
    ...
    def allocate_slots(
        ...
        recurrent_prefill_checkpoint_plan: tuple[int, int, tuple[int, ...]] | None = None,
        recurrent_checkpoint_publications: tuple[int, ...] = (),
    ) -> KVCacheBlocks | None:
        from vllm.v1.core.recurrent_prefill_checkpoint import validate_plan
        managers = self.coordinator.single_type_managers
        plan = recurrent_prefill_checkpoint_plan
        if plan is None:
            if recurrent_checkpoint_publications:
                raise ValueError("interior publication requires an explicit checkpoint plan")
            return self._allocate_slots_without_checkpoint_plan(request=request, num_new_tokens=num_new_tokens, num_new_computed_tokens=num_new_computed_tokens, new_computed_blocks=new_computed_blocks, num_lookahead_tokens=num_lookahead_tokens, num_external_computed_tokens=num_external_computed_tokens, delay_cache_blocks=delay_cache_blocks, num_encoder_tokens=num_encoder_tokens, full_sequence_must_fit=full_sequence_must_fit, reserved_blocks=reserved_blocks, has_scheduled_reqs=has_scheduled_reqs, recurrent_publication_boundary=recurrent_publication_boundary)
        start, end, targets = plan
        if (start != 0 or request.num_computed_tokens != 0 or end != request.num_prompt_tokens
                or end != request.num_tokens or num_new_tokens != end
                or num_new_computed_tokens or num_external_computed_tokens or delay_cache_blocks
                or num_encoder_tokens or (new_computed_blocks is not None and any(new_computed_blocks.blocks))):
            raise ValueError("coalesced checkpoint allocation requires a fresh complete prompt")
        if any(p not in targets for p in recurrent_checkpoint_publications):
            raise ValueError("publication boundary is not an explicitly produced internal state")
        selected = [manager for manager in managers if hasattr(manager, "_allocated_block_reqs")]
        if not selected:
            raise ValueError("checkpoint plan has no recurrent manager")
        for manager in selected:
            validate_plan(plan, start, end, manager.block_size)
            if (manager.kv_cache_spec.num_prefill_checkpoint_blocks < len(targets)
                    or manager.req_to_blocks.get(request.request_id)
                    or request.request_id in manager._allocated_block_reqs
                    or request.request_id in manager._partial_hit_reqs):
                raise ValueError("checkpoint plan cannot append a fresh recurrent table")
        try:
            for manager in selected:
                manager._planned_recurrent_checkpoints[request.request_id] = plan
                manager._planned_recurrent_publications[request.request_id] = recurrent_checkpoint_publications
            return self._allocate_slots_without_checkpoint_plan(request=request, num_new_tokens=num_new_tokens, num_new_computed_tokens=num_new_computed_tokens, new_computed_blocks=new_computed_blocks, num_lookahead_tokens=num_lookahead_tokens, num_external_computed_tokens=num_external_computed_tokens, delay_cache_blocks=delay_cache_blocks, num_encoder_tokens=num_encoder_tokens, full_sequence_must_fit=full_sequence_must_fit, reserved_blocks=reserved_blocks, has_scheduled_reqs=has_scheduled_reqs, recurrent_publication_boundary=recurrent_publication_boundary)
        finally:
            for manager in selected:
                manager._planned_recurrent_checkpoints.pop(request.request_id, None)
                manager._planned_recurrent_publications.pop(request.request_id, None)

    def _allocate_slots_without_checkpoint_plan(
        self,
        request: Request,
        num_new_tokens: int,
        num_new_computed_tokens: int = 0,

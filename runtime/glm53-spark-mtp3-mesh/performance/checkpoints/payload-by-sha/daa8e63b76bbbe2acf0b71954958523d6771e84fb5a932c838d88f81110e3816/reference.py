...
def recurrent_kda(
    ...
    *,
    ...
    captured by that same recurrence; the legacy single-offset return is unchanged.
    """
...
def prefill_kda(
    ...
    *,
    ...
    for request, (start, end) in enumerate(spans):
        ...
        for checkpoint_slot, offset in zip(slots, offsets):
    ...
    try:
        for request, (start, end) in enumerate(spans):
            ...
            for local in range((length + chunk - 1) // chunk):
                ...
                for checkpoint_index, (checkpoint_slot, offset) in enumerate(zip(slots, offsets)):

...
def validate_metadata(*, cu_seqlens, initial_state_indices, final_state_indices,
                      checkpoint_state_indices, checkpoint_offsets, num_seqs,
                      num_tokens, token_capacity, seq_capacity, state_slots,
                      chunk=16, null_state_index=None, max_checkpoints=1):
    ...
    its positive offset still must be aligned and within the request, matching
    the original device validator. Two enabled, non-null exports need distinct
    offsets and globally unique destinations. Initial may alias only own final.
    """
    ...
    for seq in range(num_seqs):
        ...
        for cp in range(max_checkpoints):
            slot = int(checkpoint_state_indices[seq] if max_checkpoints == 1
                       else checkpoint_state_indices[seq][cp])
            offset = int(checkpoint_offsets[seq] if max_checkpoints == 1
    ...
    for seq in range(num_seqs):
        ...
        if not null(initial) and initial in writes - ({final} if not null(final) else set()):
            raise ValueError("initial slot conflicts with a checkpoint or another request write")
    return spans

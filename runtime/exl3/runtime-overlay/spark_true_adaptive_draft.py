...
* only the source hashes audited for the deployed Eldritch/B12X runtime are
  accepted;
* one audited contract must be attested: maximum 4 / ladder 2,4, or maximum
  3 / ladder 2,3; both use adaptive window 32;
* adaptive proposal depths must be in ``[1, configured maximum]``;
...
def _attested_contract() -> tuple[int, int, tuple[int, ...]]:
    ...
    contract = (maximum, window, depths)
    if contract not in _SUPPORTED_CONTRACTS:
        if maximum == 4 and window == 32:
            raise RuntimeError(
                f"{_DEPTHS_ENV} must attest exactly 2,4, got {depths}"
            )
        raise RuntimeError(
            "unsupported adaptive-draft contract: "
            f"maximum={maximum}, window={window}, depths={depths}; "
            "supported contracts are maximum=3/depths=2,3/window=32 and "
            "maximum=4/depths=2,4/window=32"
        )
...
def _patch_classes(
    ...
    *,
    attested_maximum: int = 4,
) -> None:
    """Patch the three narrow seams; separated for CPU-only unit tests."""

    global _installed
    global _patched_originals
    if attested_maximum not in {3, 4}:
        raise ValueError("attested_maximum must be 3 or 4")
    with _lock:
        if _installed:
            return
    ...
    def execute_model(
        ...
        if maximum != attested_maximum:
            _record_failure()
            raise RuntimeError(
                "true adaptive drafting requires model-runner maximum depth "
                f"{attested_maximum}, "
                f"got {maximum}"
            )
    ...
    def propose(self: Any, *args: Any, **kwargs: Any) -> Any:
        ...
        if maximum != attested_maximum:
            _record_failure()
            raise RuntimeError(
                "true adaptive drafting expected speculator depth "
                f"{attested_maximum}, got {maximum}"
            )
...
def install() -> bool:
    ...
    if not _strict_enabled():
        return False
    maximum, window, depths = _attested_contract()
    ...
    _patch_classes(
        ...
        attested_maximum=maximum,
    )
    LOGGER.warning(
        "Spark true adaptive drafting installed: maximum=%d depths=%s window=%d",
        maximum,

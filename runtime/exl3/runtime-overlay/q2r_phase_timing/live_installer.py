...
_SUPPORTED_SPECULATIVE_STEPS = frozenset((3, 4, 5))
_SUPPORTED_ADAPTIVE_DEPTHS = {
    3: (2, 3),
    4: (2, 4),
}
...
def _expected_speculative_steps() -> int:
    ...
    try:
        ...
        raise RuntimeError(
            "VLLM_SPARK_MTP_TOKENS must be 3, 4, or 5"
        ) from error
    if value not in _SUPPORTED_SPECULATIVE_STEPS:
        raise RuntimeError("VLLM_SPARK_MTP_TOKENS must be 3, 4, or 5")
    return value
...
def _depth_attestation() -> SpeculativeDepthAttestation:
    ...
    expected_depths = _SUPPORTED_ADAPTIVE_DEPTHS.get(configured)
    if expected_depths is None:
        raise RuntimeError(
            "adaptive window 32 requires VLLM_SPARK_MTP_TOKENS=3 or 4"
        )
    ...
    try:
        ...
        raise RuntimeError(
            "VLLM_ADAPTIVE_SPEC_DEPTHS must attest exactly "
            + ",".join(str(depth) for depth in expected_depths)
        ) from error
    if parsed != expected_depths:
        raise RuntimeError(
            "VLLM_ADAPTIVE_SPEC_DEPTHS must attest exactly "
            + ",".join(str(depth) for depth in expected_depths)
        )

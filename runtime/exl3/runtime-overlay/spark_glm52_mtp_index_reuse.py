...
but the V2 ``MTPSpeculator`` neither compacts the final-token rows nor toggles
``set_skip_topk``.
...
_SUPPORTED_SOURCES: tuple[_SourceContract, ...] = (
    _SourceContract(
        ...
        fingerprints={
            ...
            "DeepSeekMultiTokenPredictor.compact_topk_indices": (
                "73dcc019b8cd789f62853db411d0ef61b8246670cb233b50301fee630bdce106"
            ),
...
def _fingerprint_bindings(bindings: _RuntimeBindings) -> dict[str, str]:
    ...
    return {
        ...
        "DeepSeekMultiTokenPredictor.compact_topk_indices": _sha256_source(
            predictor.compact_topk_indices
        ),
...
def _validate_runtime_contract(
    ...
    if draft_inner is not None and not callable(
        getattr(draft_inner, "compact_topk_indices", None)
    ):
        errors.append("draft model has no callable compact_topk_indices")
    ...
    if not buffer_holders:
        ...
        if len(identities) != 1:
            ...
            if dtype is None or not str(dtype).endswith("int32"):
                errors.append(
                    f"draft topk_indices_buffer dtype is {dtype!r}, not int32"
                )
            expected_rows = getattr(speculator, "max_num_tokens", None)
            if (
                shape is not None
                and expected_rows is not None
                and int(shape[0]) < int(expected_rows)
            ):
                errors.append(
                    "draft topk_indices_buffer first dimension is "
                    f"{int(shape[0])}; expected at least {int(expected_rows)}"
                )
            expected_device = getattr(speculator, "device", None)
            actual_device = getattr(draft_buffer, "device", None)
            if (
                expected_device is not None
                and actual_device != expected_device
            ):
                errors.append(
                    "draft topk_indices_buffer device is "
                    f"{actual_device!r}, not speculator device "
                    f"{expected_device!r}"
                )
            is_contiguous = getattr(draft_buffer, "is_contiguous", None)
            if not callable(is_contiguous):
                errors.append(
                    "draft topk_indices_buffer has no callable is_contiguous"
                )
            elif not is_contiguous():
                errors.append("draft topk_indices_buffer is not contiguous")
            stride = getattr(draft_buffer, "stride", None)
            if not callable(stride):
                errors.append("draft topk_indices_buffer has no callable stride")
            elif shape is not None:
                actual_stride = tuple(int(value) for value in stride())
                expected_stride = (int(shape[-1]), 1)
                if actual_stride != expected_stride:
                    errors.append(
                        "draft topk_indices_buffer stride is "
                        f"{actual_stride!r}, not {expected_stride!r}"
                    )
...
def _arm(speculator: object, skip: bool) -> None:
    ...
    speculator._spark_mtp_index_reuse_armed = skip
    with _LOCK:
        if skip:
...
def _prefill_num_reqs(args: tuple[object, ...], kwargs: Mapping[str, object]) -> int:
    raw = kwargs.get("num_reqs", args[0] if args else None)
    if not isinstance(raw, int) or raw <= 0:
        raise RuntimeError(
            "MTP index reuse requires _prefill num_reqs as a positive int; "
            f"got {raw!r}"
        )
    return raw


def _compact_for_reuse(
    speculator: object,
    args: tuple[object, ...],
    ...
    if draft_inner is None:
        raise RuntimeError("MTP index reuse was not activated for this speculator")
    num_reqs = _prefill_num_reqs(args, kwargs)
    max_num_reqs = getattr(speculator, "max_num_reqs", num_reqs)
    if num_reqs > int(max_num_reqs):
        raise RuntimeError(
            f"MTP index reuse num_reqs {num_reqs} exceeds capacity "
            f"{int(max_num_reqs)}"
        )
    last_token_indices = getattr(speculator, "last_token_indices", None)
    if last_token_indices is None:
        raise RuntimeError("MTP index reuse has no last_token_indices buffer")
    slot_ids = last_token_indices[:num_reqs]
    draft_inner.compact_topk_indices(slot_ids)
    with _LOCK:
...
def _install(
    ...
    with _LOCK:
        ...
        original_capture = auto_cls.capture
        original_prefill = auto_cls._prefill
        original_multi_step = auto_cls._multi_step_decode
        ...
        def _prefill(self, *args, **kwargs):
            ...
            try:
                ...
                _compact_for_reuse(self, args, kwargs)
        ...
        def capture(self, *args, **kwargs):
            ...
            try:
                ...
                if active:
                    # Decode graphs intentionally capture the reuse branch, but
                    # no direct/profile forward after capture may inherit that
                    # request-specific runtime state.
                    _arm(self, False)
                    with _LOCK:
                        _COUNTERS.capture_resets += 1

        def _multi_step_decode(self, *args, **kwargs):
            if (
                getattr(self, "_spark_mtp_index_reuse_active", False)
                and not getattr(self, "_spark_mtp_index_reuse_armed", False)
            ):
                # FULL prefill graph replay bypasses the Python _prefill
                # wrapper. Its captured graph still computes and compacts the
                # rows, while this arm selects the reuse branch for any eager
                # decode fallback. FULL decode graphs already have that branch
                # baked during capture.
                _arm(self, True)
            return original_multi_step(self, *args, **kwargs)
        ...
        capture.__name__ = original_capture.__name__
        capture.__doc__ = original_capture.__doc__
        _prefill.__name__ = original_prefill.__name__
        _prefill.__doc__ = original_prefill.__doc__
        _multi_step_decode.__name__ = original_multi_step.__name__
        _multi_step_decode.__doc__ = original_multi_step.__doc__
        ...
        previous: dict[str, object] = {
            ...
            "capture": mtp_cls.__dict__.get("capture", _MISSING),
            "_prefill": mtp_cls.__dict__.get("_prefill", _MISSING),
            "_multi_step_decode": mtp_cls.__dict__.get(
                "_multi_step_decode",
                _MISSING,
            ),
        ...
        mtp_cls.capture = capture
        mtp_cls._prefill = _prefill
        mtp_cls._multi_step_decode = _multi_step_decode

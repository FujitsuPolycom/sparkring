...
def _try_load_fp8_indexer_wk(
    ...
    is_scale_inv = "weight_scale_inv" in name or (
        name.endswith(".weight_scale")
        and tensor.dtype in (torch.float32, torch.float16, torch.bfloat16)
    )  # compressed-tensors block-fp8 indexer scale (RedHat NVFP4-FP8)

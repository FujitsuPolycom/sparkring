#!/usr/bin/env python3
"""Verify a GLM-5.3 ARM64 runtime image and write a source-verification receipt."""
...
def verify_image(engine: str, image: str, pins_path: Path) -> dict[str, Any]:
    ...
    if (not isinstance(observed_sources, dict) or set(observed_sources) != set(expected_sources)
            or any(not isinstance(observed_sources[name], dict)
                   or any(observed_sources[name].get(key) != value for key, value in expected.items())
